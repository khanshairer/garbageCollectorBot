...
export class World {
  ...
  init() {
    ...
    // load model and attach to bot by removing its default mesh and adding the model as a child of the bot's mesh
   ...
   // Create a charger
    ...
    // make 16 trashes at the beginning of the simulation
    this.MakeMessyAround(16);
...
// create objects at random locations in the scene 
MakeMessyAround(n, minDistance = 1.5, specify_type = null) {
  ...
  usedPositions.push({ x: this.trash_bin.position.x, z: this.trash_bin.position.z });
  usedPositions.push({ x: this.charger.position.x, z: this.charger.position.z });
